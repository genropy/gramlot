...
The complete example files are included in
...
The following listings are included so this guide can be shared as one Markdown file. To run the examples, save each listing under `docs/examples/components/component-guide/` in a Gramlot checkout with its dependencies installed. The framework itself is not embedded in this document.

### greeting-collection.js
...
<html lang="en">
  <meta charset="utf-8">
  <title>Minimal Web Component</title>
  <body>
    <gnr-guidegreeting name="Ada"></gnr-guidegreeting>
    <script type="module">
      import {defineGreeting} from './greeting-element.js';
      defineGreeting();
      document.querySelector('gnr-guidegreeting').addEventListener('greet', event => {
        console.log('Greeting requested for', event.detail.name);
      });
    </script>
  </body>
